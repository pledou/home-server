...
Grafana uses Authentik OAuth for SSO with automatic role assignment based on group membership. Users are automatically assigned roles when they log in based on their Authentik groups.

### Automatic Role Assignment

When a user logs in to Grafana via Authentik OAuth:
...
- **No grafana-\* group** → **Denied access** (cannot log in)
...
**Important**: Users MUST be in at least one grafana-\* group to access Grafana. Users without any grafana-\* group membership will be denied access.

#### Step 3: User Logs In to Grafana

When a user logs in via Authentik OAuth:
1. User is authenticated through Authentik
2. User's group membership is checked
3. Role is automatically assigned based on group (Admin/Editor/Viewer)
4. If no grafana-\* group membership, access is denied
5. User is created in Grafana on first successful login
6. Role updates automatically on each login if group membership changes
...
OAuth credentials are automatically generated by the playbook and stored securely:
- Client ID and secret are created via Authentik API
- Credentials are stored in `/home/{{ ansible_user }}/home-server/stacks/monitoring/.secrets/`
- Docker Compose reads these credentials to configure Grafana
